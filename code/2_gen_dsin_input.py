...
if __name__ == "__main__":
    ...
    user_hist_session = {}
    FILE_NUM = len(
        list(filter(lambda x: x.startswith('user_hist_session_' + str(FRAC) + '_dsin_'), os.listdir('../sampled_data/'))))
    FILE_NUM = 2
    ...
    for row in tqdm(sample_sub[['user', 'time_stamp']].iterrows()):
        sess_input_dict_, sess_input_length_dict_, sess_length = gen_sess_feature_dmsn(
            row)
    ...
    sparse_features = ['userid', 'adgroup_id', 'pid', 'cms_segid', 'cms_group_id', 'final_gender_code', 'age_level',
                       'pvalue_level', 'shopping_level', 'occupation', 'new_user_class_level', 'campaign_id', 'customer']
    ...
    for i in tqdm(range(SESS_COUNT)):
        ...
        for feat in sess_feature:
            sess_input.append(pad_sequences(
                sess_input_dict[sess_name][feat], maxlen=10, padding='post'))
