    ...
    if user not in user_hist_session or len(user_hist_session[user]) == 0:
        ...
        for i in reversed(range(len(cur_sess))):
            if cur_sess[i][2] < time_stamp:
                ...
                sess_input_length = len(sess_input_dict['brand'])
                break
    return sess_input_dict['cate_id'], sess_input_dict['brand'], neg_sess_input_dict['cate_id'], neg_sess_input_dict['brand'], sess_input_length
...
if __name__ == "__main__":
    ...
    user_hist_session = {}
    FILE_NUM = len(
        list(filter(lambda x: x.startswith('user_hist_session_' + str(FRAC) + '_din_'), os.listdir('../sampled_data/'))))
    #FILE_NUM = 2
